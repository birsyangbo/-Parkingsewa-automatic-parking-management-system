...
VEHICLE_CLASSIFICATION_MODEL = 'yolov8n.pt' 
...
# --- PARKING CAPACITY (NEW) ---
MAX_PARKING_CAPACITY = 10 # Set your desired maximum capacity here
...
ocr_reader = None
engine = None 
# Global variable for tracking last frame save time
...
# --- NEW FUNCTION: Get Current Parked Count ---
def get_current_parked_count():
    """Queries the database and returns the number of vehicles currently parked."""
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT COUNT(id) FROM parking_records WHERE status = 'parked'")
...
def record_exit(record_id):
    ...
    duration = (exit_time - entry_time).total_seconds() # Duration in seconds
    
    # --- COST CALCULATION LOGIC ---
    cost_per_second = 0 # Default if no match
    
    # Convert duration to minutes for cost calculation, ensuring a minimum charge if needed
    duration_minutes_for_cost = max(1, duration / 60) # Ensure at least 1 minute for charge
    ...
    if vehicle_type: # Ensure vehicle_type is not None
        ...
        if vehicle_type_lower == "car":
            ...
            cost_per_minute = 5
        elif vehicle_type_lower in ["bus", "truck"]:
            cost_per_minute = 20
        else: # For any other detected type or 'unknown'
            cost_per_minute = 15 # Default for 'Rest' (unknown, other large vehicles)
    else:
        cost_per_minute = 15 # Default if vehicle type is not determined

    cost = duration_minutes_for_cost * cost_per_minute
    ...
    conn.commit()
    conn.close()
    logging.info(f"Exit recorded for ID:{record_id}. Vehicle Type: {vehicle_type}. Duration: {duration/60:.2f} min, Cost: {cost:.2f}")
...
def save_plate_image(image_np):
    ...
    filename = f"plate_{timestamp}.png" # Using PNG for better quality, or JPG if smaller size is critical
...
def preprocess_image(image_path, for_cnn=True):
    try:
...
def get_vehicle_hashes(record_id=None, status=None, since_time=None,
                       include_cnn_features=False, rehash_image=False):
    conn = get_db_connection()
    cursor = conn.cursor()
    query = """
    ...
    for row in results:
        ...
        try:
            record_data['hash_value'] = imagehash.hex_to_hash(hash_str)
        except ValueError as e:
            logging.warning(f"Invalid hash for record {record_id}: {e}")
            record_data['hash_value'] = None

        if rehash_image and os.path.exists(image_path):
            preprocessed = preprocess_image(image_path, for_cnn=False)
            if preprocessed:
                record_data['rehash_value'] = imagehash.dhash(preprocessed)
            else:
                record_data['rehash_value'] = None

        if include_cnn_features and os.path.exists(image_path):
            cnn_input = preprocess_image(image_path, for_cnn=True)
            if cnn_input is not None:
                try:
                    embedding = cnn_model.predict(cnn_input, verbose=0)[0]
                    record_data['cnn_embedding'] = embedding
                except Exception as e:
                    logging.warning(f"CNN failed for {image_path}: {e}")
                    record_data['cnn_embedding'] = None
            else:
                record_data['cnn_embedding'] = None
...
def generate_ticket(record_id):
    ...
    os.makedirs(TICKET_IMAGES_DIR, exist_ok=True) # Ensure tickets directory exists
    ...
    plate_number_ocr, entry_time_str, exit_time_str, duration_seconds, cost, vehicle_type = record
...
def main_loop():
    ...
    global cap, last_save_time, engine 
    ...
    try:
        while True:
            ...
            if not matched_existing_vehicle and stable_detection_info:
                # --- START OF NEW CODE FOR PARKING FULL CHECK ---
                current_parked_count = get_current_parked_count()
                if current_parked_count >= MAX_PARKING_CAPACITY:
                    current_status_message = f"PARKING FULL: Cannot process new entry for '{stable_detection_info['ocr_text']}'. Current vehicles: {current_parked_count}/{MAX_PARKING_CAPACITY}"
                    logging.info(current_status_message)
                    speak_message("Parking is full. Please see for other parking.")
                    # Clear pending data if any, to avoid starting a new entry
                    pending_entry_data = {
                        'plate_ocr': None, 'vehicle_type': None, 'first_seen': None, 'last_seen': None, 'hash': None, 'collected_hashes': []
                    }
                    # Skip the rest of the entry logic for this frame if full
                    continue # This will go to the next iteration of the while True loop
                # --- END OF NEW CODE FOR PARKING FULL CHECK ---
...
if __name__ == "__main__":
    print("🔍 Testing get_vehicle_hashes...")

    try:
        results = get_vehicle_hashes(
            status='exited',
            include_cnn_features=True,
            rehash_image=True
        )

        for i, record in enumerate(results[:3]):  # Print first 3 for test
            print(f"\n🚗 Record {i+1}")
            print("Record ID:", record['record_id'])
            print("Plate:", record['plate_number'])
            print("Original Hash:", record.get('hash_value'))
            print("Rehash Value:", record.get('rehash_value'))
            print("CNN Embedding Length:", len(record['cnn_embedding']) if record.get('cnn_embedding') is not None else "N/A")
    except Exception as e:
        print("❌ Test failed:", e)
